...
def main():
    ...
    agent_name = 'Pat'
    booking_link = 'oncehub.com/booking_link'
    booked_status = 'not booked'
    reschedule_link = 'N/A'
    lead_first_name = st.text_input('Lead First Name', value = 'John')
    temperature = st.select_slider('Temperature', options=[round(i * 0.1, 1) for i in range(11)], value = 0.0)
    ...
    initial_text = initial_text.format(lead_first_name = lead_first_name, agent_name = agent_name)
    ...
    if st.button("Increment Day"):
        ...
        if int(day.my_var) <= 7:
            followup_message = bot_info['followup_prompt']
            newline = secret_message(followup_message)
            with open('database.jsonl', 'a') as f:
                f.write(json.dumps(newline) + '\n')
            ...
            for message in messages[1:]:
                if "secret message "  not in str(message):
                    string = string + message["role"] + ": " + message["content"] + "\n\n"
            st.write(string)
            print(string)
        else:
